...
-----------------------------------------------
  ...
  Author : Pulakesh
 Solution Idea :
                Think greedy and consider forward and reverse both traversal
                Yes, It's String based don't panic. 
...
bool solve(){
    ...
    string ans=str;
    for(int i=n-1;i>0;i--){
        int t=str[i]-'0' + str[i-1]-'0';
        //cerr<<t<<'\n';
        if(!flag && str[i]!='0' && str[i-1]!='0' && t>=10){
                ans[i-1]='1';
                t-=10;
                ans[i]=t+'0';
                i--;
    ...
    string s=str.substr(1,n-1);
    int t=str[0]-'0'+str[1]-'0';
    s[0]=t+'0';
    cout<<s;
