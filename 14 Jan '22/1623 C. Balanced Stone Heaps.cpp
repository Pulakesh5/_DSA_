...
-----------------------------------------------
 ...
 /*
    Solution idea:
                Search the height by binery search
                check in the array if x possible;
                if h[i]<x then false;
                else
                    d=min(h[i], updated[i]-height)/3;
                    h[i-1]+=d; 
                    h[i-2]+=2*d
 */
-----------------------------------------------
...
bool check(vector<int> &a,int height){
    int n=a.size();
    vector<int> arr(all(a));
...
bool solve(){
    ...
    int ans=0;
    int l=0,r= *max_element(all(arr)); 
    while(l<r){
        int height=l+(r-l+1)/2;
        //cerr<<"h "<<height<<'\n';
        if(check(arr,height))
        {
            ans=max(ans,height);
